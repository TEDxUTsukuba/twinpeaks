  ...
  <b-navbar class="is-fixed-top is-black is-spaced" id="navbar-top">
    <template slot="brand">
      <b-navbar-item tag="router-link" :to="localePath('/')">
        <img src="~/assets/logo/u_bgblack_oneline.png" alt="TEDxUTsukuba" />
      </b-navbar-item>
    </template>
    <template slot="end" v-if="!this.$route.path.includes('private')">
      <b-navbar-dropdown
        collapsible
        :label="$t('header.events.parent')"
        class="is-hoverable"
      >
        ...
        <b-navbar-item
          ...
          :key="item.path"
          tag="router-link"
          :to="{ path: localePath('/conferences/' + item.path) }"
        >
          {{ item.name }}
        </b-navbar-item>
        <b-navbar-item tag="div">
        ...
        <b-navbar-item
          tag="router-link"
          :to="{ path: localePath('/salon/2023-1') }"
        >
          2023 ほげほげ 〜まち編〜
        </b-navbar-item>
        <b-navbar-item
      ...
      <b-navbar-dropdown
        collapsible
        :label="$t('header.aboutus.parent')"
        class="is-hoverable"
      >
        <b-navbar-item
          tag="router-link"
          :to="{ path: localePath('/about/ted-and-tedx') }"
        >
          {{ $t("header.aboutus.child1") }}
        </b-navbar-item>
        <b-navbar-item
          tag="router-link"
          :to="{ path: localePath('/about/utsukuba') }"
        >
          {{ $t("header.aboutus.child2") }}
        </b-navbar-item>
        <b-navbar-item
          tag="router-link"
          :to="{ path: localePath('/about/member') }"
        >
      ...
      <b-navbar-item tag="div">
        <div class="buttons is-right">
          <nuxt-link
            ...
            ><i class="mdi mdi-translate mr-1" />English</nuxt-link
          >
          <nuxt-link
            class="button is-danger is-outlined"
            v-if="$i18n.locale !== 'ja'"
            :to="switchLocalePath('ja')"
            ><i class="mdi mdi-translate mr-1" />日本語</nuxt-link
          >
