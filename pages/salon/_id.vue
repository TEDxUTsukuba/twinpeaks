  ...
  <section id="wrapper-dark">
    ...
    <section class="section has-text-centered-mobile">
      <h1 class="subtitle is-4 is-family-narrow has-text-primary">
        TEDxUTsukubaSalon {{ salon.name }}
      </h1>
      <h1 class="title is-1">{{ salon.theme }}</h1>
      <p v-if="salon.date">
        <span class="is-family-narrow">Date</span>:
        {{ formatDate(salon.date, $i18n.locale) }}
        <span v-if="salon.startTime">{{ formatTime(salon.startTime) }}</span
        ><span v-if="salon.endTime"> - {{ formatTime(salon.endTime) }}</span>
      </p>
      <p v-if="salon.eventDelivery">
        <span class="is-family-narrow">Event Delivery</span>:
        {{ salon.eventDelivery }}
      </p>
      <p v-if="salon.location">
        <span class="is-family-narrow">Place</span>: {{ salon.location }}
      </p>
      <a
        v-if="salon.webcastUrl"
        class="button is-rounded is-gradient mt-4"
        :href="salon.webcastUrl"
        target="_blank"
        ><span v-if="salon.webcastService">{{ salon.webcastService }}で</span
        >{{ $t("about.ted.watch") }}</a
      >
    </section>
    ...
    <section class="section" v-if="salon.messageFromOrganizer">
      <article
        ...
        v-html="salon.messageFromOrganizer"
      />
      <p class="mt-3 has-text-right">
        TEDxUTsukuba {{ salon.name }}<br />Organizer<br />{{ salon.organizer }}
      </p>
    </section>
    ...
    <section class="hero" v-if="salon.timetable">
      <div class="hero-body">
        <section class="section">
          ...
          <div
            v-for="(item, index) in salon.timetable"
            :key="index"
      ...
      <div class="hero-body">
        <section class="section">
          ...
          <div v-if="salon.hasTicket" class="mb-4">
            <p class="mb-3 notes">
              Price: <strong>JPY {{ salon.ticketPrice }}</strong>
              <span v-if="salon.ticketPriceStudent"
                >(Student: <strong>{{ salon.ticketPriceStudent }}</strong
                >)</span
              >
            </p>
            ...
            <div v-if="salon.isTicketAvailable">
              ...
              <a
                v-if="salon.ticketSellingPageUrl"
                class="button is-gradient is-rounded my-3"
                :href="salon.ticketSellingPageUrl"
                target="_blank"
              >
            ...
            <div v-else>
              <span class="tag is-success"
                ...
                {{ formatDate(salon.ticketReleaseDate) }}</span
              >
            </div>
    ...
    <section
      ...
      :style="
        salon.locationImage
          ? `background-image: url(${salon.locationImage.url});`
          : ''
      "
    >
      <div class="hero-body">
    ...
    <section v-if="salon.locationGooglemapsEmbed">
      <figure class="image is-3by1">
        <iframe
          class="has-ratio"
          :src="salon.locationGooglemapsEmbed"
          width="100%"
          rameborder="0"
  ...
  head() {
    if (!this.ready) {
      ...
      return {
        ...
        description:
          this.salon.socialMediaDescription || this.salon.themeStatement,
        meta: [
          {
            hid: "og:title",
            property: "og:title",
